#!/usr/bin/env python3
"""
Launch random permutation ablation on RunPod.
1 pod x 3 sequential jobs (seeds 0, 7, 8 @ 200K steps each).

Estimated: ~140 min total (~$1.40 at $0.59/hr Secure Cloud).

This ablation applies a fixed random permutation to the 82-dim observation
vector, reordering dimensions without mixing them. Unlike random orthogonal
projection (which mixes all features), permutation only changes which index
each feature lives at.

Prediction: permutation should NOT improve meso-scale metrics (PaCMAP R²,
TriMap R²) the way orthogonal projection did, because permutation preserves
anisotropic scatter structure — it reorders corridors but doesn't blend them.
If true, this confirms that feature mixing (not just coordinate independence)
is the mechanism behind the random projection's cleaner manifold geometry.
"""
...
# Job definitions: (job_id, steps, arrangement, mask_count, mask_slots, shuffle_blobs, random_project, random_permute, seed)
JOBS = [
    (f"ablation_randperm_s{seed}", STEPS, "grid", "false", "false", "false", "false", "true", seed)
    for seed in SEEDS
]
...
def make_start_command(jobs):
    ...
    for job_id, steps, arrangement, mask_count, mask_slots, shuffle_blobs, random_project, random_permute, seed in jobs:
        cmds.append(f"echo '=== Starting {job_id} ==='")
        cmds.append(
            f"bash /workspace/bridge/cloud/cloud_run.sh {seed} {steps} {job_id} "
            f"{arrangement} {mask_count} {mask_slots} {shuffle_blobs} {random_project} {random_permute}"
        )
...
def main():
    print("=" * 60)
    print("RANDOM PERMUTATION ABLATION — RunPod Launch")
    print("=" * 60)
    print(f"Seeds: {SEEDS}")
    ...
    cmd_file = Path(__file__).parent / "pod_randperm_cmd.sh"
    with open(cmd_file, "w") as f:
        f.write("#!/bin/bash\n")
        f.write("# Auto-generated: random permutation ablation\n")
    ...
    if not runpod.api_key:
        ...
        print("You can manually run pod_randperm_cmd.sh on a RunPod pod.")
        return
    ...
    try:
        ...
        print("  3. SSH in and run: bash /workspace/bridge/cloud/pod_randperm_cmd.sh")
        print("  4. Results will be in /workspace/results/*_ghe.json")
    except Exception as e:
        print(f"Failed to create pod: {e}")
        print("You can manually create a pod and run pod_randperm_cmd.sh")
