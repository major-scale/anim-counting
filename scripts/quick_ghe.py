#!/usr/bin/env python3
"""
Quick GHE check — runs eval on CPU in the container against a checkpoint.
Does NOT interfere with Mac training. Takes ~3-5 minutes.

Usage:
    python quick_ghe.py <checkpoint_dir> [--episodes_per 5]
"""
...
def run_eval(agent, blob_counts, episodes_per, seed=42):
    """Run eval episodes and collect h_t + counts."""
    ...
    for n_blobs in blob_counts:
        ...
        for ep in range(episodes_per):
            ...
            while not done:
                vec = obs_raw["vector"]
                # Read count from env state directly (not obs vector)
                # so it works correctly when obs indices 80-81 are masked
...
def compute_ghe(h_t, counts):
    """Compute GHE, arc-length R², and persistent homology."""
...
def main():
    ...
    # Get training step from metrics
    ...
    print(f"=" * 50)
    print(f"Quick GHE Check ({device})")
    print(f"=" * 50)
