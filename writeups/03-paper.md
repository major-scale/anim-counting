...
We test this by building a *second* counting environment in which the same concept (how many blobs have been gathered) is implemented as a 4-bit binary register with cascading carry propagation. Incrementing 0→1 flips one bit; incrementing 7→8 flips four. Training the same DreamerV3 architecture on this environment produces a dramatically different representational geometry — organized by Hamming distance rather than ordinal distance. More specifically, the natural step vectors of the binary successor decompose linearly onto four orthogonal bit-flip directions in the recurrent state, with 100% sign agreement across all 15 transitions (0→1 through 14→15), cross-talk ~0.001 on unchanged bits, and the decomposition survives both held-out generalization (probes trained without a subset of transitions correctly classify the withheld transitions, including the maximally adversarial 7→8 full cascade) and a Makelov subspace-illusion test (rowspace-only intervention reproduces the full-direction effect). The same model internally simulates the multi-step carry cascade autonomously in imagination, bit-by-bit from LSB to MSB, matching the posterior update schedule and anticipating full-cascade transitions up to 8 steps before onset.

**The central empirical result is this compositional coordinate system, validated at a methodological bar (held-out generalization across all transitions + Makelov-faithful against subspace-illusion alternatives) that is unusual for compositional-linearity claims in learned world models — where prior work has largely relied on correlational evidence without faithfulness validation.** The complementary smooth-manifold geometry in the counting-grid environment supports a broader interpretive claim we develop in Section 9: representation geometry in deep-RL world models tracks the task's physical instantiation, not the abstract concept being represented. This interpretation has a published lineage (Alleman-Lindsey-Fusi 2024; Driscoll-Shenoy-Sussillo 2024; Zhong et al. 2023); our contribution is the minimal-pair controlled instantiation in a deep recurrent world model, with the same abstract quantity held fixed across environments.
...
### Why the compositional geometry is not trivially task-imposed

A natural objection to any orthogonal-decomposition claim is that the orthogonality is imposed by the task structure rather than discovered in the representation: the four bit columns are orthogonal in the observation by construction, so any faithful decoder will preserve that orthogonality. Three pieces of evidence argue against this reading. **(a)** Orthogonality is not forced by the observation encoder. The 512-dimensional recurrent state could have represented 16 counter states in many non-compositional ways — as a 1D manifold parameterized by integer value, for instance, which is exactly what happens in the decimal gathering environment with the same architecture (Section 5). The architecture admits multiple geometries; the physical implementation selects among them. **(b)** The decomposition generalizes to transitions held out during probe training — including the maximally adversarial 7→8 full cascade where probes had never seen any 4-bit flip (Section 6.3, `results/binary_heldout_decomposition.md`). A probes-fit-their-own-direction tautology would not predict this. **(c)** The carry-cascade anticipation dynamics use the bit-decomposition basis in a causally-implicated way, not merely as a probing artifact: imagination rollouts generate the sequential LSB→MSB cascade autonomously at the posterior's pace (Section 6.4), and single-direction intervention at α_nat is Makelov-faithful for bits 0 and 1 (bits 2 and 3 pass via the decomposition-fraction and class-separation lines when α_nat is insufficient to drive decoder flips; Appendix C).
...
- **C4.** A compositional decomposition of the binary successor into four orthogonal bit-flip directions in a deep RSSM's recurrent state: 100% sign agreement across all 15 transitions, cross-talk ~0.001, held-out generalization to transitions unseen during probe training, and Makelov-faithful single-direction intervention (rowspace-only reproduces full-direction effect). *(Section 6.3; Appendix C.)*
- **C5.** Internal simulation of carry cascades: with the RSSM forked into imagination mode (prior only, no observation updates), the agent autonomously generates the sequential LSB→MSB bit-flip cascade bit-by-bit, with timing matching the posterior update schedule and anticipation onset scaling with carry depth (up to 8 steps before natural onset in full-cascade cases). This bridges two previously disconnected literatures — feedforward carry circuits in transformer addition (Quirke & Barez, 2024) and preplay/successor-representation anticipation in the hippocampal literature. *(Section 6.4.)*
- **C6.** The FP Unifier architecture and empirical demonstration that it preserves both geometric structures simultaneously, validated with directional interchange intervention analysis and an untrained null control. *(Section 7.)*
- **C7.** Three mechanistic findings on modular integration: (i) accuracy-geometry dissociation via contrastive sweeps, (ii) late-onset > early-onset VICReg on modular architectures, (iii) cooperative residual plasticity as a discontinuity at zero backbone learning rate. *(Section 8.)*
...
The closest concurrent work is Gurnee et al. (2025), who find 1D counting manifolds in LLMs from a prediction objective. The important distinction is that our setting is *embodied sequential*: the agent physically acts in an environment and the representation emerges from next-state prediction on sensorimotor streams. LLM settings count tokens; ours counts gathered physical objects. Kantamneni & Tegmark (2025) identify helical 9D integer representations in transformers that encode base-10 structure as a single continuous geometry shared across all integers; we instead find that register-based binary counting in a recurrent world model produces a *factored* geometry — four orthogonal bit-direction axes rather than a unified curve — establishing that even within "emergent number representations" the geometric form is determined by the task's physical instantiation rather than the abstract quantity. Chawla et al. (2026) ("MetaOthello") provide a contemporary transformer-side comparison: training a single model on multiple Othello variants, they find that isomorphic token-remapped games produce representations "equivalent up to a single orthogonal rotation that generalizes across layers," with rule-conflict variants showing layer-depth-indexed divergence. Our result runs in the opposite direction: we hold the abstract quantity fixed and vary the task physics, producing representations that are *not* rotation-equivalent across environments. Together these bracket the conditions under which rotation-equivalence holds.

### 2.3 World models

DreamerV3 (Hafner et al., 2023) learns an RSSM that predicts future observations and rewards. Prior representational analyses of world models have focused on spatial navigation (Banino et al., 2018), object permanence (Piloto et al., 2022), and physical intuition (Battaglia et al., 2013). Hu et al. (2024) applied Vapnik's Learning Using Privileged Information (Vapnik & Vashist, 2009) to DreamerV3's observation space via the Scaffolder framework, showing that auxiliary sensors bridge 79% of the performance gap between target and privileged observations by scaffolding *learning*, not *representation*. Our ablation results (Section 5.3) are consistent with this interpretation. Sobotka et al. (NeurIPS 2025 MI Workshop) is the only prior work we are aware of that applies mechanistic-interpretability methods directly to a trained DreamerV3 RSSM; their setting is working-memory tasks with coordinate-level single-bit patching. We work in a counting setting and test a *compositional* claim (four orthogonal directions jointly composing a successor) rather than coordinate-level localization of a single binary variable.

### 2.4 Representational geometry

Representational similarity analysis (RSA; Kriegeskorte et al., 2008), persistent homology (Rybakken et al., 2019; Chazal & Michel, 2021), and manifold dimensionality estimation (Jazayeri & Ostojic, 2021) provide complementary views of neural geometry. Fascianelli et al. (2024) demonstrated that two monkeys performing the same task at the same accuracy level exhibited strikingly different representational geometries in PFC — an independent biological precedent for the accuracy-geometry dissociation we observe in Section 8.1. The Platonic Representation Hypothesis (Huh et al., 2024) and the Anna Karenina scenario (Bansal et al., 2021) both predict representational convergence under sufficient prediction pressure. Gröger, Wen & Brbić (2026) refine the PRH with a calibration framework (the "Aristotelian" variant): apparent global spectral convergence across modalities largely disappears once scale-based inflation is controlled for, while local-neighborhood similarity retains cross-modality agreement. Our binary-vs-decimal geometric contrast is most naturally read as a negative result against strong-PRH readings — the same abstract quantity (count) produces qualitatively different global geometries depending on task physics — while remaining compatible with the weaker Aristotelian reading at the level of local neighborhood structure.

The closest prior-art claim to our own thesis is Alleman, Lindsey & Fusi (ICLR 2024), whose title — "Task structure and nonlinearity jointly determine learned representational geometry" — is near-verbatim. We acknowledge this as an independently established direction and differentiate on mechanism. Their setting is one-hidden-layer feedforward networks; their mechanism is activation-function choice (Tanh networks learn target-output-structured representations, ReLU networks retain input-structured representations, driven by ReLU's asymmetric saturation). Our setting is a 512-dimensional deterministic recurrent state in a deep RL world model; our mechanism is the physical implementation of the task (continuous gather vs discrete register cascade) acting as an inductive bias on the learned representation. The title collision is real; the mechanistic regimes are disjoint. Driscoll, Shenoy & Sussillo (Nature Neuroscience 2024) identify reusable "dynamical motifs" — ring attractors, point attractors, decision boundaries — that multi-task RNNs share across tasks with common sub-computations. Their compositional-reuse claim operates at the level of dynamical-system primitives in the activation trajectory; ours operates at the level of a linear coordinate system in the recurrent state, with each coordinate carrying a separable causal role under Makelov-faithful intervention. Zhong et al. (2023) ("Clock and Pizza") show that a single task can produce qualitatively different representations (clock vs pizza structure) under different hyperparameter choices; we vary the task's physical implementation while holding the abstract quantity and the hyperparameters fixed.
...
### 2.7 Causal intervention and compositional features

Our compositional-decomposition result inherits evidential standards from the mech-interp literature on causally validated subspaces. Geiger et al. (2023) ("Distributed Alignment Search") establish the framework for subspace-level interchange intervention; we adopt the standard that a compositional claim requires not just linear decodability but causal efficacy of each axis under patching. Makelov, Lange & Nanda (2023) sharpen this standard by diagnosing a failure mode — a probe direction that succeeds at an intervention while being largely in the nullspace of downstream processing, with the causal work done by a disconnected component. We apply their test and show that our bit-flip directions pass it: rowspace-only intervention reproduces the full-direction effect within sampling noise (Appendix C). Wu et al. (2024) subsequently argued that null-space effects are generally expected from data-induced submanifolds and are not diagnostic of illusion. Our design sidesteps this objection by making a strictly stronger claim: rather than defending null-space components as benign, we show the rowspace-only intervention alone recovers the full-direction effect, which is stronger than what the Makelov test requires and immune to the Wu et al. counterpoint. Engels et al. (ICLR 2025) ("Not All Language Model Features Are One-Dimensionally Linear") are the nearest methodological neighbor for compositional causal features: they identify irreducible multi-dimensional features (circular day-of-week, month-of-year) in LLMs via sparse autoencoders and verify them causally on modular-arithmetic tasks. Our setting is different — a factored compositional basis of four linear axes in an RSSM rather than a 2D circle in an LLM — but the methodological move (identify structure via linear tools, validate causally) is shared. Huang, Wu, Potts, Geva & Geiger (2024) ("RAVEL") provide the closest compositional-attribute-disentanglement benchmark; our work is closer to a case study with a mechanistically simple task (4-bit carry propagation) than to a benchmark submission, but their entangled-attribute framework is the natural comparison frame. Quirke & Barez (ICLR 2024) give the closest prior art for carry-as-internal-variable: a feedforward transformer performing multi-digit addition via parallel per-digit streams with explicit carry circuits. Their carry circuit is feedforward and fails at cascade depth ≥ 3; ours is recurrent and anticipatory, generating the full 4-bit cascade autonomously from internal dynamics (Section 6.4). Nanda (2023) on grokking remains the canonical demonstration that task structure determines emergent geometry (the modular-addition circle); our work extends this lineage to the case where the task's *physical instantiation*, not just its abstract structure, is the determining factor.
...
The bit-decomposition basis is not only a static characterization of the posterior representation — it is used by the RSSM's generative dynamics. The agent's imagination rollouts internally simulate carry cascades bit-by-bit from LSB to MSB, with timing matched to the posterior update schedule and anticipation beginning up to 8 steps before the natural transition in full-cascade cases.
...
The primary empirical result is the compositional decomposition of the binary successor: four orthogonal bit-flip directions in the recurrent state, capturing the natural step vectors with 100% sign agreement on all 15 transitions, generalizing to transitions held out during probe training, and passing the Makelov subspace-illusion test (rowspace-only intervention reproduces the full-direction effect). The agent's imagination rollouts use this basis causally, generating the sequential LSB→MSB carry cascade from internal dynamics alone at the posterior's pace. Taken together this is a compositional coordinate system for binary arithmetic inside a deep-RL world model, validated at an unusually rigorous methodological bar for learned representations.

The complementary counting-grid result adds a minimal-pair controlled comparison: same architecture, same training objective, same abstract concept (count), different physical implementation (continuous gather vs discrete register cascade), qualitatively different learned geometry (smooth 1D arc vs factored Hamming hypercube). We offer this as a minimal-pair controlled mechanistic instantiation of a broader direction that has published antecedents: task structure and architecture jointly shape representation geometry (Alleman, Lindsey & Fusi 2024); shared dynamical motifs mirror task structure in multi-task RNNs (Driscoll, Shenoy & Sussillo 2024); different hyperparameters produce qualitatively different geometries within a single task (Zhong et al. 2023). We did not propose this direction; we provide a controlled instantiation in a new regime (deep-RL world model, same abstract quantity varied across physical implementations) and a mechanistic characterization of both endpoints.

The contrast is also readable as a negative result against strong readings of the Platonic Representation Hypothesis (Huh et al. 2024). Strong-PRH would predict that sufficient prediction pressure on the same abstract quantity should drive representations toward a shared geometry; we find the opposite at the global-geometry level. This is compatible with the Aristotelian refinement (Gröger, Wen & Brbić 2026), which after scale-based calibration retains cross-modality convergence at the level of local neighborhood structure rather than global geometry. Our binary and decimal manifolds are globally incompatible but may share local neighborhood relations in the regions where they overlap — a direction we leave for follow-up.

Auxiliary observation channels scaffold learning *reliability* without determining converged geometry (mean GHE drifts by only 0.04 across starvation conditions while seed-variance grows from 0.027 to 0.081). The format of the observation matters more than its content — coordinate-structure disruption via permutation or projection cleans up meso-scale representation quality in ways invisible to standard metrics (Section 5.4).
...
**Cross-seed basis alignment of the bit-flip directions is not evaluated.** The counting-manifold topology is established across 5 seeds (Section 5); the binary specialist's compositional basis is currently established on a single seed (`binary_baseline_s0`). Whether the specific four-axis basis aligns across independently trained instances — or whether only the span does, with the specific orthogonal frame reorienting per-seed — is a future analysis. The held-out generalization and Makelov-faithfulness results establish that the basis identified on this seed is not an artifact of probe fitting within a single distribution, but that is a different claim from cross-seed uniqueness of the basis itself.
...
We have shown that a DreamerV3 world model trained on a 4-bit binary counting environment develops a compositional coordinate system in its 512-dimensional recurrent state: four orthogonal bit-flip directions that capture the natural successor step-vectors with 100% sign agreement across all 15 transitions, generalize to transitions held out during probe training, pass the Makelov subspace-illusion test under rowspace-only intervention, and are used causally by the RSSM's imagination dynamics to autonomously generate the sequential LSB→MSB carry cascade at the posterior's pace. A complementary experiment on decimal counting in the same architecture produces a qualitatively different geometry — a smooth 1D manifold with formally verified arc topology and uniform geodesic spacing — supporting the broader interpretive claim that representation geometry in deep-RL world models tracks the task's physical implementation, not the abstract concept being represented. We offer this as a minimal-pair controlled instantiation of a direction with published antecedents (Alleman-Lindsey-Fusi 2024; Driscoll-Shenoy-Sussillo 2024; Zhong et al. 2023), not as a de novo thesis.

Naïve combined training fails to preserve both geometries; the FP Unifier — frozen specialists plus learned adapters plus a shared integrator — succeeds, validated causally at 84.2% directional IIA on an 8D subspace. The mechanism of integration produced three findings of independent interest: a contrastive-alignment sweep reveals a sharp phase transition at λ = 0.005 in which geometric structure collapses while task accuracy is perfectly preserved (a computational demonstration of the accuracy-geometry dissociation reported in monkey PFC, Fascianelli et al. 2024); brief VICReg variance pressure applied late (after the GRU has consolidated) outperforms the same pulse applied early, extending the critical learning periods framework to modular architectures where differential plasticity between components breaks the standard prediction; and explicitly freezing the GRU during late-onset VICReg reverses the effect, revealing a discontinuity at exactly zero backbone learning rate that we call cooperative residual plasticity — a phenomenon predicted by low-rank RNN theory and two-timescale stochastic approximation but not previously named or unified.

Integration — combining representations that exist in different geometries of the same concept — is a distinct problem from learning, with its own mechanisms, timing dynamics, and failure modes. Counting is a clean window into all of them.
...
- Alleman, M., Lindsey, J. W., & Fusi, S. (2024). Task structure and nonlinearity jointly determine learned representational geometry. *ICLR 2024*. arXiv:2401.13558.
...
- Chawla, H., Hall, B., & Lovato, P. (2026). MetaOthello: A Controlled Study of Multiple World Models in Transformers. arXiv:2602.23164.
...
- Driscoll, L. N., Shenoy, K. V., & Sussillo, D. (2024). Flexible multitask computation in recurrent networks utilizes shared dynamical motifs. *Nature Neuroscience*.
- Engelken, R., Wolf, F., & Abbott, L. F. (2023). Lyapunov Spectra of Chaotic Recurrent Neural Networks. *Physical Review Research*.
- Engels, J., Michaud, E. J., Liao, I., Gurnee, W., & Tegmark, M. (2025). Not All Language Model Features Are One-Dimensionally Linear. *ICLR 2025*. arXiv:2405.14860.
...
- Geiger, A., Wu, Z., Lu, H., Rozner, J., Kreiss, E., Icard, T., Goodman, N., & Potts, C. (2023). Finding Alignments Between Interpretable Causal Variables and Distributed Neural Representations (DAS). *CLeaR 2024*. arXiv:2303.02536.
...
- Huang, J., Wu, Z., Potts, C., Geva, M., & Geiger, A. (2024). RAVEL: Evaluating Interpretability Methods on Disentangling Language Model Representations. arXiv:2402.17700.
...
- Quirke, P., & Barez, F. (2024). Understanding Addition in Transformers. *ICLR 2024*. arXiv:2310.13121.
...
- Sobotka, F., et al. (2025). Mechanistic Interpretability of a DreamerV3 World Model. *NeurIPS 2025 Mechanistic Interpretability Workshop*.
...
- Wu, Z., Geiger, A., Huang, J., Arora, A., Icard, T., Potts, C., & Goodman, N. D. (2024). A Reply to Makelov et al. (2023)'s "Interpretability Illusion" Arguments. arXiv:2401.12631.
