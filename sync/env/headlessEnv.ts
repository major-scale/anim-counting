...
import {
  ALL_PERSONALITIES,
  type BotPersonality,
} from "./counting-world/botPersonalities.js";
import {
  createBot,
  updateBot,
  type Bot,
} from "./counting-world/bot.js";
import { type Vec2, vec2, dist, length } from "./counting-world/steering.js";
import {
  type Blob,
  type BlobField,
  countBlob,
...
} from "./counting-world/countingStrategy.js";
import {
...
export interface EnvConfig {
  ...
  blobCountMin: number;
  blobCountMax: number;
  /** Max steps before truncation (safety valve) */
  maxSteps: number;
}
...
export interface EpisodeState {
  bot: Bot;
  blobField: BlobField;
  strategy: CountingStrategy | null;
  phase: "counting" | "predict";
  step: number;
  done: boolean;
  truncated: boolean;
  botTotalDistance: number;
  prevBotPos: Vec2;
  blobCountAtStart: number;
  botType: string;
...
function botStateValue(bot: Bot): number {
  if (bot.allVisited) return 2; // done
  if (bot.pauseTimer > 0) return 1; // counting/paused at blob
  return 0; // seeking
}

export function getObservation(state: EpisodeState): number[] {
...
function selectBotPersonality(stage: 1 | 2 | 3 | 4 | 5): BotPersonality {
  const byName = (name: string) => ALL_PERSONALITIES.find((p) => p.name === name)!;
  switch (stage) {
    case 1:
...
function createArrivalHandler(
  personalityName: string,
  field: BlobField,
): (bot: Bot, waypointIndex: number) => boolean {
  switch (personalityName) {
    case "confused": {
      const stopAfter = field.totalCount + 3 + Math.floor(Math.random() * 4);
      return (bot, idx) => {
        const blob = field.blobs[idx];
        if (!blob) return true; // blob removed (conservation OFF)
...
function createBlobFieldFromPositions(positions: Vec2[]): BlobField {
  const BLOB_COLORS = [
    "#F9A8D4", "#FCA5A5", "#FDBA74", "#FCD34D",
    "#86EFAC", "#67E8F9", "#A5B4FC", "#C4B5FD",
  ];

  const blobs: Blob[] = positions.map((pos, i) => ({
    id: i,
    position: { ...pos },
    radius: 10 + Math.random() * 8,
    baseColor: BLOB_COLORS[i % BLOB_COLORS.length],
    countedBy: new Set<string>(),
    markedBy: null,
    isCarried: false,
    carriedBy: null,
    placedPosition: null,
...
function mutateBlobs(field: BlobField): void {
...
export function resetEnv(config: EnvConfig): EpisodeState {
  // Random blob count
  ...
  const positions = generateArrangement(
    blobCount, WORLD_WIDTH, WORLD_HEIGHT, MARGIN, MIN_SEPARATION, arrangementType,
  );
  ...
  let strategy: CountingStrategy | null = null;
...
export function stepEnv(state: EpisodeState, config: EnvConfig, action: number | null): StepResult {
  ...
  if (!bot.allVisited) {
    ...
    if (strategy) {
      if (strategy.phase === "done") {
        ...
        const target = tickStrategy(
          bot, state.blobField, strategy, WORLD_WIDTH, WORLD_HEIGHT, placementFn,
          strategyArrivalRadius,
        );
    ...
    // World boundary push
    if (bot.position.x < BOUNDARY_MARGIN) bot.velocity.x += 0.3;
    if (bot.position.x > WORLD_WIDTH - BOUNDARY_MARGIN) bot.velocity.x -= 0.3;
    if (bot.position.y < BOUNDARY_MARGIN) bot.velocity.y += 0.3;
    if (bot.position.y > WORLD_HEIGHT - BOUNDARY_MARGIN) bot.velocity.y -= 0.3;
  }
...
function buildInfo(state: EpisodeState, config: EnvConfig): Record<string, unknown> {
  return {
